...
<template>
  <section class="bg-gradient-warm border-b border-border/60">
    <div class="container py-12 sm:py-16 md:py-24">
      <p class="text-xs uppercase tracking-[0.25em] text-muted-foreground">Discover</p>
      <h1 class="font-display text-4xl sm:text-5xl md:text-6xl mt-3 max-w-2xl">A quiet directory of trusted specialists.</h1>
      <div class="mt-8 sm:mt-10 flex flex-col md:flex-row gap-3 max-w-3xl">
        ...
        <button class="h-12 rounded-full px-8 bg-primary text-primary-foreground w-full md:w-auto">Search</button>
      </div>
    </div>
  </section>
  <section class="container py-10 sm:py-12">
    <div class="mb-8 sm:mb-10">
      ...
      <div class="mt-3 flex gap-2 overflow-x-auto pb-2">
        <button class="rounded-full border px-4 py-1.5 text-sm transition whitespace-nowrap" :class="selectedSpecialty ? 'border-border text-muted-foreground hover:text-foreground hover:border-foreground/40' : 'bg-primary text-primary-foreground border-primary'" @click="setQueryParam('specialty', '')">All specialties</button>
        <button v-for="specialty in specialties" :key="specialty" class="rounded-full border px-4 py-1.5 text-sm transition whitespace-nowrap" :class="selectedSpecialty === specialty ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:text-foreground hover:border-foreground/40'" @click="setQueryParam('specialty', specialty)">{{ specialty }}</button>
      </div>
      <div class="mt-2 flex gap-2 overflow-x-auto pb-2">
        <button class="rounded-full border px-4 py-1.5 text-sm transition whitespace-nowrap" :class="selectedCity ? 'border-border text-muted-foreground hover:text-foreground hover:border-foreground/40' : 'bg-primary text-primary-foreground border-primary'" @click="setQueryParam('city', '')">All cities</button>
        <button v-for="city in cities" :key="city" class="rounded-full border px-4 py-1.5 text-sm transition whitespace-nowrap" :class="selectedCity === city ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:text-foreground hover:border-foreground/40'" @click="setQueryParam('city', city)">{{ city }}</button>
      </div>
    </div>
    <div class="flex items-baseline justify-between mb-6 sm:mb-8"><h2 class="font-display text-xl sm:text-2xl">{{ results.length }} specialists</h2></div>
